 ...
 *
 * Roda por `pnpm tsx scripts/prova-agenda.ts` (que chama o vitest com
 * `vitest.prova.config.ts`) contra uma pilha local com o baseline aplicado.
 * `PROVA_MANTER=1` não apaga a org de prova; `PROVA_GOOGLE=1` empurra ao
 * Google Calendar conectado.
 */
import { createClient } from "@supabase/supabase-js";
import pg from "pg";
import { describe, it } from "vitest";
import { anunciarDestino, credenciaisSupabaseDeTeste } from "../../scripts/lib/env-de-teste";
import { afirmar, passo, pular } from "../../scripts/lib/prova";
...
describe("prova-agenda — da mensagem clínica redigida à consulta paga", () => {
  it("roda a jornada inteira", async () => {
  ...
  await passo("Mensagem clínica entra redigida e a conversa vai para humano", async () => {
    ...
    const [msg] = await q(
      `select body, metadata from public.messages where organization_id = $1 and direction = 'inbound' order by created_at desc limit 1`,
      [orgId],
    );
