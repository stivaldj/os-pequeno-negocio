 ...
 *
 *  1. Quem monta modelo FORA do worker (a sugestão de funil do onboarding)
 *     tinha um segundo switch, `buildModel()` no motor antigo, que ficou três
 *     casos atrás do registry. O fork apagou o motor; o que vigia agora é que
 *     ninguém reabra um switch paralelo — todo caminho deriva do registry.
 ...
 *     valor.
 *
 * Os testes abaixo fecham a corrente: lista → registry de produção → o único
 * chamador fora do worker → o que a tela realmente renderiza.
 */
describe("a corrente inteira: lista × execução × tela", () => {
  it("o onboarding monta modelo pelo registry, não por um switch próprio", async () => {
    // Guarda de construção: o segundo switch morreu com o motor antigo, e a
    // única forma de ele voltar é alguém escrever `case "anthropic"` de novo
    // fora de providers.ts. O único chamador fora do worker é a sugestão de
    // funil do onboarding.
    const { readFileSync } = await import("node:fs");
    const fonte = readFileSync("app/actions/onboarding/montarQuadro.ts", "utf8");
    expect(fonte).toContain("createDefaultRegistry()");
    expect(fonte).not.toMatch(/case\s+["'](anthropic|openai|google|openrouter)["']/);
  });

  it("e o registry continua recusando provedor que ninguém declarou (a catraca não virou peneira)", () => {
    // Sem isto, trocar o registry por um `createOpenAI` universal faria o teste
    // do topo passar e aceitaria qualquer string como provedor.
    expect((registry as Record<string, unknown>)["provedor-que-nao-existe"]).toBeUndefined();
  });
