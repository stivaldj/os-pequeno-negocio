...
export function meetPgSupabase(pool: pg.Pool) {
  ...
  class Query implements PromiseLike<Result> {
    ...
    async execute(): Promise<Result> {
      try {
        ...
        let sql: string;
        if (this.mode === "select")
          sql = `select * from ${table}${where}${this.limitN === undefined ? "" : ` limit ${this.limitN}`}`;
      ...
      try {
        ...
        const result = await pool.query(
          ...
          entries.map(([key, value]) => {
            // Esta porta recebe text[]; os demais argumentos preservam JSONB.
            if (name === "fn_reply_record_receipt" && key === "p_echo_ids" && Array.isArray(value))
              return value;
