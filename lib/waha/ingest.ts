...
async function handleInbound(
  ...
  await markConversation(admin, session.organization_id, conversationId, "inbound", preparada.redigido ? preparada.preview : previewFromMessage(p), p.timestamp ? new Date(p.timestamp * 1000).toISOString() : now);
